classdef pl < hop.hop
% PhaseLift class definition (Michael Friedlander, 2016)
% Modified to track method runtimes (Will Wright, 2017, denoted ~WW)
   ...
   methods
      ...
      function out = forward(self,x1,x2,scatter)
         ...
         self.tForward = self.tForward + toc(tFwd);
      ...
      function out = adjoint(self,y,x)
         ...
         self.tAdjoint = self.tAdjoint + toc(tAdj);
         self.nAdjoint = self.nAdjoint + 1;
      end % adjoint
      
      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

      function [f,g,V,d,infoEigsFFTs,eigsIterData] = gdobjective(self,y,k,opts,v)
         tGdo = tic;
         ...
         eigsopts.isreal = self.isrealin;
				 % Returns iterate data from 'eigs'.  Requires modified version of 'eigs'.  ~WW
         if ~isempty(opts) && isfield(opts, 'returnEigsIterData') && opts.returnEigsIterData
            eigsopts.returnIterData = true;
         else
            eigsopts.returnIterData = false;
         end
         nMultsPrev = self.nAdjoint;
			tEigs = tic;
         if eigsopts.returnIterData
            [V,d, ~, eigsIterData] = eigs_mod(Afun,self.n,k,sigma,eigsopts);
         else
            [V,d] = eigs(Afun,self.n,k,sigma,eigsopts);
			   eigsIterData = [];
         end
         nMatVec = self.nAdjoint - nMultsPrev;
         self.nMultsInEigs = self.nMultsInEigs + nMatVec;
         self.nFFTsInEigs = self.nFFTsInEigs + (2*nMatVec+1)*numel(y)/self.n;
         infoEigsFFTs.nMatVec = nMatVec;
         infoEigsFFTs.nFFTs = (2*nMatVec+1)*numel(y)/self.n;
         self.tEigs = self.tEigs + toc(tEigs);
			self.nEigs = self.nEigs + 1;
         ...
         self.tGDObjective = self.tGDObjective + toc(tGdo);
         self.nGDObjective = self.nGDObjective + 1;
      end % gdobjective
      
      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      
      function [f,g,r, H] = pfdobjective(self,rhs,x)
			   tPfdo = tic;
...
%          end
         if (nargin <= 2 || isempty(x))
            ...
            f  = dot(r(:),r(:))/4;
            if nargout >= 2
               g  = -reshape(sum(bsxfun(@times,conj(self.masks),...
                                        ifft2(bsxfun(@times,r,Ax))),...
                                 3),...
                             [self.n1,self.n2,numel(x)/self.n]);
               if self.isrealin
                  g = real(g);
               end
            end
...
% TODO: xxx Modify here
            ...
            util.addToNIFFT2(numel(Ax)/self.n);
						self.tPFDObjective = self.tPFDObjective + toc(tPfdo);
...
%          end
         ...
         self.tDFPObjective = self.tDFPObjective + toc(tDfpo);
      end % dfpobjective
