function [V, D, flag, iterData] = eigs_mod(varargin)
...
%
%   Modification made by user (Will Wright) for prototyping:
%   OPTS.returnIterData: Flag to return information about ARPACK iterations
%
...
[V, d, flag, timings, t0, numMatVecMults, iterData] ...
    = CallARPACK(applyOP, applyM, n, k, innerOpts, mode, t0, nargout);
...
if innerOpts.eigsDisplay >= 2
    printTimings(timings, mode, Amatrix, B, numMatVecMults);
end
end
...
innerOpts = struct('tol', tol, 'maxit', maxit, 'p', p, 'v0', v0, ...
    ...
    'returnIterData', returnIterData, 'ishermA', ishermA, 'isrealprob', isrealprob);
...
function [v, d, flag, timings, t0, numMatVecMults, iterData] ...
           = CallARPACK(applyOP, applyM, n, k, innerOpts, mode, t0, nOutputs)
% Send variables to ARPACK and run algorithm

iterData.types = ['Block Iteration', 'Num Subiters (A*x products)', 'ido (ARPACK) flag', 'Ritz vals'];
iterData.iter_vals = [];
iterData.Ritz_vals = [];

if innerOpts.eigsDisplay == 3
    fprintf('\n Eigs called with extended console display\n'); 
    fprintf(' k = %i, p = %i, tol = %1.4e, maxits = %i\n\n', k, ...
             innerOpts.p, innerOpts.tol, innerOpts.maxit);
    fprintf(' Iter    Num Subiters   ido (ARPACK)   Ritz vals \n');
    fprintf('        (A*x products)                 \n');
end
...
eigsIter = 0;
subIter = 0;
numMatVecMults = -1; % Balances out sum due to subIter counting intial vector v as an iterate
...
while ido ~= 99
    ...
    % NOTE: arpackc takes pointers allowing modification of v
    if isrealprob
    ...
    if double(ipntr(15)) > eigsIter || ido == 99
        if innerOpts.returnIterData
            iterData.iter_vals = double([iterData.iter_vals; ...
                                    double(ipntr(15)), subIter, ido]);
        end
        if innerOpts.eigsDisplay == 3
            fprintf(' %3i          %3i           %3i    ', double(ipntr(15)), subIter, ido);  
        end
        numMatVecMults = numMatVecMults + subIter;
        subIter = 0;
    end
    if innerOpts.eigsDisplay || innerOpts.returnIterData
        iterData = displayRitzValues(iterData, innerOpts);
    end
end % while ido ~= 99
...
if innerOpts.returnIterData
    last_Ritz_vals = flipud(d);
    [~, old_col] = size(iterData.Ritz_vals);
    [~, new_col] = size(last_Ritz_vals');
    iterData.Ritz_vals = [iterData.Ritz_vals; [last_Ritz_vals' nan(1, old_col - new_col)] ];
end
...
% Nested functions in CallARPACK
  ...
  function [iterData] =  displayRitzValues(iterData, innerOpts)
                ...
                if ishermprob
                    ...
                    if strcmp(whch,'BE')
                        ...
                        if innerOpts.eigsDisplay
                            ds = num2str(dispvec(max(end-2*k+1,1):end)', '  %1.15f');
                            disp(ds)
%                            disp(dispvec(max(end-2*k+1,1):end)')
                        end
                        if innerOpts.returnIterData
                            temp_array = dispvec(max(end-2*k+1,1):end)';
                        end
                    else
                        % k eigenvalues
                        if innerOpts.eigsDisplay
                            ds = num2str(dispvec(max(end-k+1,1):end)', '  %1.15f');
                            disp(ds)
%                            disp(dispvec(max(end-k+1,1):end)')
                        end
                        if innerOpts.returnIterData
                            temp_array = dispvec(max(end-k+1,1):end)';
                        end
                    end
                else
                    ...
                    if innerOpts.eigsDisplay
                        ds = num2str(dispvec(max(end-k,1):end)', '  %1.15f');
                        disp(ds)
%                        disp(dispvec(max(end-k,1):end)')
                    end
                    if innerOpts.returnIterData
                        temp_array = dispvec(max(end-k,1):end)';
                    end
                end
            else
...
%                    disp(dispvec(max(end-k+1,1):end)')
            ...
            iterData.Ritz_vals = [iterData.Ritz_vals; temp_array];
        end
    end
...
function printTimings(timings, mode, Amatrix, B, numMatVecMults)
...
fprintf(['\n' sstr '\n' ...
    ...
    sstr '\n'], ...
    timings(2));
fprintf('Number of matrix-vector multiplications: %i\n', numMatVecMults);  
end