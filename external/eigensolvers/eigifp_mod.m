function [EVals, EVecs, Iters, matvecTotal, iter_cache] = eigifp_mod(varargin) 
%
%  ** Modified from original file to include more outputs (Iters) - Luan
%  ** Modified to output matvecCount and iter_cache - Will Wright (~WW)
%  ** Modified to control maximum size of inner iteration block - ~WW
%
%
...
%       Default: between 1 and opt.MAXINNERIT as adaptively determined. 
%     opt.MAXINNERITERATION or opt.MAXINNERIT:         ~Added by WW
%       maximum size of inner iteration block; 
%       Default: 128. 
%     opt.USEPRECON:    
...
if ( nargin > 1 + ~isempty(B) + ksetbyuser )
    ...
    I=strmatch('MAXINNERITERATION',upper(names),'exact');
    if (isempty(I))
        I=strmatch('MAXINNERIT',upper(names),'exact');
    end
    if (~isempty(I))
        m_max=getfield(options,names{I});
        if ( m_max < 0 | m_max >= n )
                error('Input Error: Invalid inner iteration maximum number');
        end     
    end
...
if (isnumeric(A))       %if given a matrix, use (A,B) or (-A,B) if maxeig =1
    if (findMax==1)
        ...
        [EVals, EVecs, Iters, matvecTotal, iter_cache] = ifree(-A,B,n,m,m_max,toleranceVec,iterMax,k,X,eta,L,usePrecon,normA,normB,outputYes,do_iter_cache);
        if (outputYes)
    ...
    if (findMax==1)     % here we use minusA and negate the results when done
        ...
        [EVals, EVecs, Iters, matvecTotal, iter_cache] = ifree('minusA',B,n,m,m_max,toleranceVec,iterMax,k,X,eta,L,usePrecon,normA,normB,outputYes,do_iter_cache);
        if (saveAfunc==1)
...
function [lambda, x, r, bx, diff, rDiff, bDiff, lam_max, res, V_cache] = ...
     parnob(A, B, L, lambda, x, r, bx, diff, rDiff, bDiff, m, tol, do_iter_cache, ...
            EVecs, BEVecs, lambda_1, lambda_max)
%  
...
function [lambda, x, r, bx, diff, rDiff, bDiff, d2, lam_max, res, V_cache] = ...
     lancb(A, B, lambda, x, r, bx, diff, rDiff, bDiff, m, tol, do_iter_cache, ...
           EVecs, BEVecs, lambda_1, lambda_max)
%  
...
if (nargin == 16)                   % post processing Ritz vector
    for j=1:count
            temp=BEVecs(:,j)'*x;
            x=x-EVecs(:,j)*temp;
            if ( ~ isempty(B) ),
...
function [Lambda_1, EVecs, Iters, matvecTotal, iter_cache] = ...
    ifree(A, B, n, m, m_max, tol, itermax, k, X, eta, L, usePrecon, normA, normB, outputYes, do_iter_cache);
%
...
for l=1:k
    ...
    matvecTotal = matvecTotal + size(x, 2);
    matvecCount = size(x, 2);
    ...
    if (outputYes ~=0)
        ...
        fprintf(1,'  Iteration\tEigenvalue\tResidual\tNum mat-vecs (inner block size)\n');
        fprintf(1,'  ================================================================\n');
        fprintf(1,'  %d\t\t%E\t%E\t%d\n',1,lambda,res,mValue); 
    end
    ...
    for iter=2:itermax
        ...
        if (l>1)
          ...
          if ( startL == 0 )
                    [lambda, x, r, bx, diff, rDiff, bDiff, d2, lambda_max, res, iter_cache_temp] = ...
              lancb(A, B, lambda, x, r, bx, diff, rDiff, bDiff, projSize,  tol, do_iter_cache,...
                          EVecs(:,1:l-1), BEVecs(:,1:l-1), Lambda_1(1:l-1), Lambda_Max(1:l-1,:));
          else
                    preconCount=preconCount+mValue;
                    [lambda, x, r, bx, diff, rDiff, bDiff, lambda_max, res, iter_cache_temp] = ...
              parnob(A, B, L, lambda, x, r, bx, diff, rDiff, bDiff, projSize, tol, do_iter_cache,...
                          EVecs(:,1:l-1), BEVecs(:,1:l-1), Lambda_1(1:l-1), Lambda_Max(1:l-1,:));
          end           
        else
                    % no deflation if l=1. 
        ...
        matvecTotal = matvecTotal + mValue;
        if do_iter_cache
          iter_cache.V{l, iter} = iter_cache_temp;
          iter_cache.lam{l, iter} = lambda;
          iter_cache.matvecs{l, iter} = matvecCount;
        end
        
        if (outputYes ~= 0)
            fprintf(1,'  %d\t\t%E\t%E\t%d\n',iter,lambda,res,mValue);
        end
        ...
        if ( changeCounter >= 19 )
                    ...
                    [mValue, priorM, priorRate, fixM]=updateM(rate,mValue,priorM,priorRate,n,m_max);
...
function [mValue, priorM, priorRate, fixM]=updateM(rate, mValue, priorM, priorRate,n,m_max);          
%
