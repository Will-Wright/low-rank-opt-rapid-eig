...
% 9 Jan 2018: modified to terminate if solver diverges; WEW.
...
%gNormPrev = inf;                    % Set previous residual and gradient
%rNormPrev = inf;                    % to determine if flow no longer progressing
%diffTol = 1e-4;
...
while true
   ...
   elseif t >= iterations
      status = 'iterations';
      break
   elseif isnan(gNorm) || isinf(gNorm) || isnan(rNorm) || isinf(rNorm)
      status = 'diverged';
      break
%   elseif abs(gNormPrev - gNorm) < diffTol || abs(rNormPrev - rNorm) < diffTol
